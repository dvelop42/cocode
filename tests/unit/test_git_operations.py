...
class TestGitWorktree:
    ...
    def test_worktree_cleanup(self, temp_repo):
        ...
        # Remove worktree
        subprocess.run(
            ["git", "worktree", "remove", str(worktree_path), "--force"], cwd=temp_repo, check=True
        )
...
class TestGitOperations:
    ...
    def test_fetch_before_operations(self, mock_subprocess_run):
        ...
        mock_subprocess_run.assert_called_with(["git", "fetch", "--all", "--prune"])
...
class TestBranchManagement:
    ...
    def test_branch_creation_from_base(self, temp_repo):
        ...
        subprocess.run(["git", "checkout", "-b", new_branch], cwd=temp_repo, check=True)
    ...
    def test_branch_exists_check(self, mock_subprocess_run):
        ...
        mock_subprocess_run.return_value = Mock(returncode=0, stdout="cocode/123-test\n")
...
class TestGitSafety:
    ...
    def test_git_config_isolation(self, temp_repo):
        ...
        # Set local config
        subprocess.run(["git", "config", "--local", "test.key", "value"], cwd=temp_repo, check=True)

        # Verify config is set
        result = subprocess.run(
            ["git", "config", "--local", "test.key"], cwd=temp_repo, capture_output=True, text=True
        )
