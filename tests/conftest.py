...
def sample_cocode_config():
    ...
    return {
        ...
        "performance": {"max_concurrent_agents": 3, "default_timeout": 900, "polling_interval": 5},
    }
    ...
    with patch("subprocess.run") as mock_run:
        ...
        def side_effect(*args, **kwargs):
            ...
            if "gh" in cmd and "auth" in cmd and "status" in cmd:
                ...
                return Mock(
                    returncode=0,
                    stdout=json.dumps({"number": 123, "title": "Test Issue", "body": "Issue body"}),
                )
            elif "gh" in cmd and "pr" in cmd and "create" in cmd:
                return Mock(returncode=0, stdout="https://github.com/test/repo/pull/456")
